...
# Function to read and extract text from a document
...
def chat_view(request):
    ...
    if request.method == 'POST':
        ...
        if 'csv_file' in request.FILES:
            ...
            doc_content = extract_text_from_document(full_file_path)
            if isinstance(doc_content, str):
                # Clear previous messages to start a new conversation
                messages.clear()  # Clear existing messages
                messages.append({"role": "system", "content": "Document content: " + doc_content[:3000]})
